...
function basis(index: number): number[] {
  const vector = Array.from({ length: PROFILE_METADATA.embeddingDim }, () => 0);
  vector[index] = 1;
  return vector;
}
...
describe("PostgresRagDocumentStore (integration)", () => {
  ...
  it("retrieves only active chunks with the exact embedding profile in deterministic order", async () => {
    const chunks = [
      chunkCore(1, 2, "doc:v1:chunk-002"),
      chunkCore(1, 1, "doc:v1:chunk-001"),
      chunkCore(1, 3, "doc:v1:chunk-003"),
    ];
    await store.stageVersion({ version: versionCore(1), chunks });
    await store.saveChunkEmbeddings([
      embeddingCore(1, 2, { embedding: basis(0) }),
      embeddingCore(1, 1, { embedding: basis(0) }),
      embeddingCore(1, 3, { embedding: basis(1) }),
      embeddingCore(1, 1, {
        embedding: basis(2),
    ...
    const staged = chunkCore(2, 1, "doc:v2:chunk-001");
    await store.stageVersion({ version: versionCore(2), chunks: [staged] });
    await store.saveChunkEmbeddings([embeddingCore(2, 1, { embedding: basis(0) })]);
